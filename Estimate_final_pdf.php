...
$perphoto = $_REQUEST["photo"];//collect a photo permition
include "connection.php";
$obj = new conn;
$obj->update_temp_estimate($freight);
$obj->transferdata();
$p_result = $obj->temp_estimate_product_display();
$result = $obj->temp_estimate_display();
...
require_once('tcpdf/tcpdf.php');
    ...
    // $photo_path = "image/66SSJ.PNG";
        ...
        if($perphoto==1){
            ...
            $items = array(
                array(1, 1234,'Website Design', 1, 500.01, 50.03, 12 , 2000.03),
                array(2, 5456,'SEO Optimization', 1000, 200.02, 20.02, 18 ,39655.04),
            ...
            // $pdf->Cell(20, 6, 'HSN', 1, 0, 'C');
            // $pdf->Cell(20, 6, 'IMAGE', 1, 0, 'C');
            $pdf->Cell(98, 6, 'PARTION', 1, 0, 'L');//+10
            $pdf->Cell(12, 6, 'PCS', 1, 0, 'C');
            $pdf->Cell(20, 6, 'RATE', 1, 0, 'C');
            $pdf->Cell(13, 6, 'DISC%', 1, 0, 'C');//+2
            $pdf->Cell(12, 6, 'GST%', 1, 0, 'C');//+3
            $pdf->Cell(25, 6, 'AMOUNT', 1, 0, 'C');
            $pdf->Ln(); // Move to the next line
            ...
            while($item = mysqli_fetch_array($p_result)) {
                ...
                // $pdf->Cell(20, 20, $item[1], 1, 0, 'C');
                // $pdf->Cell(20, 20, '', 1, 0, 'C').$pdf->Image("image/".$item[2], 41, $pdf->GetY()+1, 18, 18);
                $pdf->MultiCell(98, 7, $item[3], 1, 'L',false,0);//+10
                $pdf->Cell(12, 7, $item[4], 1, 0, 'C');
                $pdf->Cell(20, 7, sprintf("%.2f",$item[5]), 1, 0, 'R');
                $pdf->Cell(13, 7, sprintf("%.2f",$item[6]), 1, 0, 'C');//+2
                $pdf->Cell(12, 7, sprintf("%.2f",$item[7]), 1, 0, 'C');//+3
